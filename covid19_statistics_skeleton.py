def normalize_data(n_cases, n_people, scale):
    ...
    if scale <= 0 :
        print('ERROR : the scale must be more than 0.')
        return -1
    norm_cases = []
    for idx, n in enumerate(n_cases):
        n_ratio = n/n_people[idx]*scale
        norm_cases.append(n_ratio)
    return norm_cases

def sum_data_of_list(lists):
    total = 0
    for idx in lists:
        total += idx
    return total
...
sum_people = sum_data_of_list(n_people) # TODO) The total number of people
sum_covid  = sum_data_of_list(n_covid) # TODO) The total number of new cases
...
print('| Region | COVID-19 New cases | Ratio (%) | New Cases / 1M |')
print('| ------ | --------- | --------- |--------------- |')
